...
DatabaseConfig ConfigureDatabase () {
	...
	// Configura entradas analogicas e binarias para cada slave
	 for(int i = 0; i < NUM_SLAVES ; i++) {
		 ...
		 config . analog_input [i]. svariation = (i == 0) ? StaticAnalogVariation ::Group30Var1:StaticAnalogVariation ::Group30Var2 ;
...
int main()
{
    ...
    // Tamanhos do banco (NUM_SLAVES pontos analógicos e binários)
	OutstationStackConfig stackConfig(ConfigureDatabase());


    // Se sua versão suportar, aplique o DatabaseConfig (classes/variations)
    // stackConfig.dbConfig = ConfigureDatabase();
		...
		// Atualiza valores DNP3
		{
			 ...
			 UpdateBuilder builder ;
			 {
				 ...
				 if ((* slave_states )[ slave_index ]. connection_changed ) {
					 ...
					 if ( elapsed > 100) { // Espera pelo menos 100 ms antes de resetar
...
int main () {
    ...
    auto channel = manager . AddTCPServer ("server", logLevels , ServerAcceptMode :: CloseExisting ,  IPEndpoint ("10.1.1.223",20000),
    PrintingChannelListener :: Create ());
